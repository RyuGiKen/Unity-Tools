...
using UnityEngine;
using FramingMode = Cinemachine.CinemachineGroupComposer.FramingMode;
namespace Cinemachine
{
    /// <summary>
    /// Same CinemachineGroupComposer
    /// </summary>
    public class CinemachineGroupComposerCalculate : MonoBehaviour
    {
        ...
        public bool IsValid { get { return enabled && LookAtTarget != null; } }
        private void LateUpdate()
        {
            CameraState curState = VirtualCamera.State;
            MutateCameraState(ref curState, 1f / m_FrameDamping);
            if (VirtualCamera is CinemachineFreeLook)
            {
                CinemachineFreeLook freeLook = VirtualCamera as CinemachineFreeLook;
                freeLook.m_Lens = curState.Lens;
            }
            else if (VirtualCamera is CinemachineVirtualCamera)
            {
                CinemachineVirtualCamera virtualCamera = VirtualCamera as CinemachineVirtualCamera;
                virtualCamera.m_Lens = curState.Lens;
            }
        }
